...
export default function ProfilePage2() {
  ...
  const [blogBackgrounds, setBlogBackgrounds] = useState({});
    ...
    const fetchBlogs = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Shuffle backgroundImages and assign one per blog
          const shuffledImages = [...backgroundImages].sort(() => 0.5 - Math.random());
          const blogsWithImages = data.map((blog, index) => ({
              ...blog,
              bg: shuffledImages[index % shuffledImages.length]
          }));
        ...
        <div className="m-4">
          <div className="">
            <div className="px-4 py-4 mx-2 md:mx-4 my-2 flex flex-col justify-center items-center">
              ...
              <h1 className="px-2 py-1 text-xl md:text-3xl font-serif">
                {username}
              </h1>
              <h4 className="p-1 text-sm md:text-base font-serif">
                {degree} | {batch}
              </h4>
            </div>
          </div>
